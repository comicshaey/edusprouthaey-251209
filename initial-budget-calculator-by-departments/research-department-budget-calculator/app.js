...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function updateAll() {
    ...
    rows.forEach(tr => {
      ...
      if (amt > 0) {
        amtTd.textContent = rdFmtMoney(amt);
    ...
    CATS.forEach(c => {
      ...
      if (sum > 0) lines.push(`<p>· ${c.label}: <b>${rdFmtMoney(sum)}</b></p>`);
      else lines.push(`<p>· ${c.label}: 0원</p>`);
    });
    lines.push("<hr>");
    lines.push(`<p><b>총 소요 예산(연구부)</b> = <b>${rdFmtMoney(grandTotal)}</b></p>`);
  ...
  function makeNote() {
    ...
    CATS.forEach(c => {
      const sum = catSum[c.value];
      if (sum > 0) catLines.push(`${c.label} ${rdFmtMoney(sum)}`);
    });

    const detailLines = details.map(d => {
      const label = CATS.find(c => c.value === d.cat)?.label || d.cat;
      const notePart = d.note ? `, 비고: ${d.note}` : "";
      return `- [${label}] ${d.name}: 단가 ${rdFmtMoney(d.unitVal)} × ${d.qtyVal} = ${rdFmtMoney(d.amt)}${notePart}`;
    });
    ...
    const html = `
      <p>
        ...
        연구활동비·자료제작비 등으로 총 <b>${rdFmtMoney(grandTotal)}</b>을 편성하고자 합니다.${writerTxt}
      </p>
      <p>
